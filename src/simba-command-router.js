  ...
  async handleMessage({ chatId, text }) {
    ...
    try {
      ...
      if (trimmed.startsWith("/registry")) {
        await this.messenger.sendMessage(chatId, "🔍 Scanning live decide.engine-tools registry...");
        try {
          const tools = await fetchAllRegisteredTools(this.config);
          const report = formatRegistryReport(tools);
          await this.messenger.sendMessage(chatId, truncateForTelegram(report));
        } catch (err) {
          await this.messenger.sendMessage(
            chatId,
            errMsg("Registry scan failed", err.message, true, "Check GITHUB_TOKEN and network.")
          );
        }
        return;
      }
      ...
      if (trimmed.startsWith("/gaps")) {
        ...
        // Filter catalog to requested categories if specified
        const catalogToCheck = categories
          ? Object.fromEntries(
              Object.entries(TOOL_CATALOG).filter(([cat]) => categories.includes(cat))
            )
          : TOOL_CATALOG;

        await this.messenger.sendMessage(chatId, "🔍 Checking live registry for missing tools...");
        try {
          const missing = await discoverMissingTools(catalogToCheck, this.config);
          const report = formatMissingToolsReport(missing);
          await this.messenger.sendMessage(chatId, truncateForTelegram(report));
        } catch (err) {
          await this.messenger.sendMessage(
            chatId,
            errMsg("Gaps check failed", err.message, true, "Check GITHUB_TOKEN and network.")
          );
        }
        return;
      }
